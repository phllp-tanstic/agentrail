...
_resetRiskState();
const a1 = checkPreOrder({ marketId: M1, estimatedSpendUsd: 0.9 });
check(1, 'small order within limits is allowed', a1.allow === true,
  `allow=${a1.allow} reservationId=${a1.reservationId}`);
releaseReservation({ reservationId: a1.reservationId, why: 'test cleanup' });

_resetRiskState();
const a2 = checkPreOrder({ marketId: M1, estimatedSpendUsd: 12.76 });
...
check(3, 'a REFUSED order reserves nothing', _openReservations().length === 0 && riskSnapshot().reservedUsd === 0,
  `openReservations=${_openReservations().length} reservedUsd=${riskSnapshot().reservedUsd}`);

// Aggregation: five 0.9 orders commit 4.5, a sixth would reach 5.4 > 5.
_resetRiskState();
for (let i = 0; i < 5; i++) {
  const r = checkPreOrder({ marketId: M1, estimatedSpendUsd: 0.9 });
  commitReservation({ reservationId: r.reservationId, actualSpentUsd: 0.9 });
}
const a4 = checkPreOrder({ marketId: M1, estimatedSpendUsd: 0.9 });
...
const a5 = checkPreOrder({ marketId: M2, estimatedSpendUsd: 0.9 });
...
releaseReservation({ reservationId: a5.reservationId, why: 'test cleanup' });

// Daily loss: drive drawdown to 9.6 across separate markets so the WINDOW cap
// (5) is not what trips — isolating the daily cap.
_resetRiskState();
for (let i = 0; i < 12; i++) recordSpend({ marketId: `0xm_${i}`, spentUsd: 0.8 });
const ddBefore = riskSnapshot().drawdownUsd;
const a6 = checkPreOrder({ marketId: '0xm_fresh', estimatedSpendUsd: 0.9 });
...
recordPayout({ marketId: '0xm_0', payoutUsd: 5.0 });
const ddAfter = riskSnapshot().drawdownUsd;
const a7 = checkPreOrder({ marketId: '0xm_fresh', estimatedSpendUsd: 0.9 });
...
releaseReservation({ reservationId: a7.reservationId, why: 'test cleanup' });

_resetRiskState();
const a8 = checkPreOrder({ marketId: M1, estimatedSpendUsd: 'not-a-number' });
...
_resetRiskState();
const b1 = checkPreOrder({ marketId: M1, estimatedSpendUsd: 2.0 });
const snapB1 = riskSnapshot();
...
async function racingOrder(tag, usd) {
  const rk = checkPreOrder({ marketId: M1, estimatedSpendUsd: usd });
  ...
  commitReservation({ reservationId: rk.reservationId, actualSpentUsd: usd });
...
const finalExposure = riskSnapshot().perMarketExposureUsd[M1] ?? 0;
...
_resetRiskState();
const b4 = checkPreOrder({ marketId: M1, estimatedSpendUsd: 4.0 });   // worst case
commitReservation({ reservationId: b4.reservationId, actualSpentUsd: 0.101229 });
const snapB4 = riskSnapshot();
...
const b4b = checkPreOrder({ marketId: M1, estimatedSpendUsd: 4.5 });
...
releaseReservation({ reservationId: b4b.reservationId, why: 'test cleanup' });

// ---- a reverted placement: commit(0) is a full release with no spend
_resetRiskState();
const b5 = checkPreOrder({ marketId: M1, estimatedSpendUsd: 3.0 });
commitReservation({ reservationId: b5.reservationId, actualSpentUsd: 0 });
const snapB5 = riskSnapshot();
...
// ---- explicit release
_resetRiskState();
const b6 = checkPreOrder({ marketId: M1, estimatedSpendUsd: 3.0 });
releaseReservation({ reservationId: b6.reservationId, why: 'refused after reserving (slippage)' });
const snapB6 = riskSnapshot();
...
_resetRiskState();
const b7 = checkPreOrder({ marketId: M1, estimatedSpendUsd: 2.0 });
commitReservation({ reservationId: b7.reservationId, actualSpentUsd: 1.5 });
const afterFirst = riskSnapshot().spendUsd;
const dbl = commitReservation({ reservationId: b7.reservationId, marketId: M1, actualSpentUsd: 1.5 });
const afterSecond = riskSnapshot().spendUsd;
...
_resetRiskState();
const b8 = checkPreOrder({ marketId: M1, estimatedSpendUsd: 2.0 });
let resolved = false;
const guardedCommit = (usd) => {
  if (!b8.reservationId || resolved) return false;
  commitReservation({ reservationId: b8.reservationId, marketId: M1, actualSpentUsd: usd });
  resolved = true; return true;
};
guardedCommit(1.5); const g1 = riskSnapshot().spendUsd;
guardedCommit(1.5); const g2 = riskSnapshot().spendUsd;
...
_resetRiskState();
const b9 = checkPreOrder({ marketId: M1, estimatedSpendUsd: 4.0 });
// deliberately never resolved
const leaked = checkPreOrder({ marketId: M1, estimatedSpendUsd: 2.0 });
check(22, 'an UNRESOLVED reservation permanently eats capacity — the reason for the try/finally backstop',
  leaked.allow === false && _openReservations().length === 1,
...
releaseReservation({ reservationId: b9.reservationId, why: 'simulating the backstop' });
const recovered = checkPreOrder({ marketId: M1, estimatedSpendUsd: 2.0 });
check(23, 'the backstop release fully restores that capacity',
  recovered.allow === true && riskSnapshot().reservedUsd === 2,
  `after release, the same 2.0 USD order is allowed`);
releaseReservation({ reservationId: recovered.reservationId, why: 'test cleanup' });

// ---- daily cap counts reservations too
_resetRiskState();
const held = [];
for (let i = 0; i < 5; i++) {
  const r = checkPreOrder({ marketId: `0xdm_${i}`, estimatedSpendUsd: 1.9 });
  if (r.allow) held.push(r.reservationId);
}
const snapDaily = riskSnapshot();
const dailyBlocked = checkPreOrder({ marketId: '0xdm_last', estimatedSpendUsd: 1.9 });
...
for (const id of held) releaseReservation({ reservationId: id, why: 'test cleanup' });
check(26, 'releasing all of them restores the full daily budget',
  riskSnapshot().drawdownUsd === 0 && riskSnapshot().remainingDailyLossBudgetUsd === DAY,
  `drawdown=${riskSnapshot().drawdownUsd} remaining=${riskSnapshot().remainingDailyLossBudgetUsd}`);

// ---- the ledger records every transition
_resetRiskState();
const b10 = checkPreOrder({ marketId: M1, estimatedSpendUsd: 2.0 });
commitReservation({ reservationId: b10.reservationId, actualSpentUsd: 1.0 });
const b11 = checkPreOrder({ marketId: M1, estimatedSpendUsd: 1.0 });
releaseReservation({ reservationId: b11.reservationId, why: 'audit trail test' });
recordPayout({ marketId: M1, payoutUsd: 0.5 });
const kinds = _riskEvents().map((e) => e.kind);
...
const commitEvent = _riskEvents().find((e) => e.kind === 'commit');
